...
{
    public class LoadLog0 : BaseUnityPlugin
    {
        public enum Mode { Load = 0, Import = 1 };
        // Token: 0x0600000D RID: 13 RVA: 0x00002A78 File Offset: 0x00000C78
        public void Awake()
        {
            LoadLog0.Debug("Loading RockyScript!");
            ExtendedSave.SceneBeingLoaded += LoadLog0.ExtendedSceneLoad;
            ExtendedSave.SceneBeingImported += LoadLog0.ExtendedSceneImport;
            ExtendedSave.SceneBeingSaved += LoadLog0.ExtendedSceneSave;
        }
        ...
        internal static void LoadInfo(Mode mode)
        {
            ...
            {
                bool flag1 = sceneExtendedDataById.data.ContainsKey("GifInfo");
                bool flag2 = sceneExtendedDataById.data.ContainsKey("Mp4Info");
                bool flag3 = sceneExtendedDataById.data.ContainsKey("MonInfo");
                if (mode == Mode.Import && (flag1 || flag2 || flag3))
                {
                    try
                    {
                ...
                if (flag1)
                {
                    ...
                    foreach (StudioGifResolveInfo studioGifResolveInfo in list)
                    {
                        ...
                        {
                            LoadLog0.Debug("Gif Plane: Unknown Mode!\n");
                        }
                ...
                if (flag2)
                {
                    ...
                    foreach (StudioMp4ResolveInfo studioMp4ResolveInfo in list2)
                    {
                        ...
                        {
                            LoadLog0.Debug("Mp4 Plane: Unknown Mode!\n");
                        }
